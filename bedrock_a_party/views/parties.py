...
from bedrock_a_party.classes.party import CannotPartyAloneError, NotExistingFoodError, NotInvitedGuestError, ItemAlreadyInsertedByUser, Party
...
def all_parties():
    ...
    if request.method == 'POST':
        try:
            guests = request.get_json('guests')
            result = create_party(jsonify(guests))
...
def edit_foodlist(id, user, item):
    ...
    if 'POST' == request.method:
            try:
                _LOADED_PARTIES[str(id)].add_to_food_list(item, user)
                result = jsonify(_LOADED_PARTIES[str(id)].get_food_list().serialize()['food'==item and 'user'==user])
